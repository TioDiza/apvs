      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 max-w-4xl">
        <div className="text-center mb-16">
          <Reveal>
            ...
            <h3 className="text-3xl md:text-4xl font-extrabold text-gray-900">Perguntas Frequentes</h3>
          </Reveal>
        </div>
            ...
            <Reveal key={index} delay={index * 100}>
              <div 
                className={`border rounded-xl overflow-hidden transition-all duration-300 ${
                  openIndex === index ? 'border-apvs-green-500 shadow-md bg-white' : 'border-gray-200 bg-gray-50 hover:border-gray-300'
                }`}
              >
                <button
                ...
                >
                  <span className={`font-semibold text-lg ${openIndex === index ? 'text-apvs-blue-900' : 'text-gray-800'}`}>
                    {faq.question}
                  </span>
                  <ChevronDown 
                    className={`w-5 h-5 transition-transform duration-300 ${openIndex === index ? 'transform rotate-180 text-apvs-green-500' : 'text-gray-400'}`} 
                  />
                </button>
                <div 
